...
# Topics Mapping: key -> (topic_id, term_id, title, order_no)
TOPICS = {
...
for item in tf2_matches:
    ...
    if num <= 30:
        top_key = 't2_carrying_capacity'
    elif num <= 60:
        top_key = 't2_biotechnology'
    elif num <= 90:
        top_key = 't2_plate_tectonics'
    elif num <= 120:
        top_key = 't2_global_climate'
    elif num <= 150:
        top_key = 't2_global_interactions'
    else:
        top_key = 't2_sustainability'
    add_q(top_key, 2, q_txt, 'True', 'False', None, None, ans, f'The statement is {ans}.')
...
for i in range(1, len(parts_tf1), 3):
    ...
    for q_match in re.finditer(r'(\d+)[\.\)]\s*(.*?)(?=\n\s*\d+[\.\)]|\n\s*Answer|\Z)', q_sec, re.DOTALL):
        ...
        q_body = re.sub(r'\s+', ' ', q_match.group(2)).strip()
...
def parse_id_file(fname, part_map):
    ...
    for i in range(1, len(parts), 3):
        ...
        for a_match in re.finditer(r'(\d+)[\.\)]\s*([^\n]+)', ans_sec):
            ...
            a_val = a_match.group(2).strip()
            # Clean up trailing punctuation
            a_val = re.sub(r'\s*\.$', '', a_val)
            ans_map[q_n] = a_val
            
        for q_match in re.finditer(r'(\d+)[\.\)]\s*(.*?)(?=\n\s*Answer:|\n\s*\d+[\.\)]|\Z)', q_sec, re.DOTALL):
            q_num = int(q_match.group(1))
            q_body = re.sub(r'\s+', ' ', q_match.group(2)).replace('Answer:', '').strip()
...
def parse_mcq_term1():
    ...
    for i in range(1, len(groups), 3):
        ...
        for b in blocks:
            q_stem = re.sub(r'\s+', ' ', b[0]).strip()
            ca = b[1].strip()
            cb = b[2].strip()
            cc = b[3].strip()
            cd = b[4].strip()
            ans_let = b[5].strip().upper()
            exp = b[6].strip()
            add_q(top_key, 1, q_stem, ca, cb, cc, cd, ans_let, exp)
...
def parse_mcq_term3():
    reader = PdfReader(os.path.join(assets_dir, 'MCQ-term-3.pdf'))
    text = '\n'.join([p.extract_text() or '' for p in reader.pages])
    groups = re.split(r'Group\s+(\d+)[·\s]+([^\n]+)', text)
    ...
    for i in range(1, len(groups), 3):
        g_num = int(groups[i])
        g_content = groups[i+2]
        top_key = top_map.get(g_num, 't3_projectile_motion')
        ...
        for b in blocks:
            ...
            def clean_opt(o):
                return o.replace('✅', '').replace('*', '').strip()
...
parse_mcq_term2()
parse_mcq_term3()
print(f"Loaded {len(records)} total questions after all MCQ files.")

# --- 6. Generate Master SQL Script ---
with open('seed_master_questions.sql', 'w', encoding='utf-8') as out:
    out.write("-- ====================================================================\n")
    out.write("-- MASTER QUESTION SEED DATA FOR SUPABASE DATABASE\n")
    out.write("-- Generated from Assets PDFs & DOCX files\n")
    out.write("-- ====================================================================\n\n")
...
print(f"Successfully generated seed_master_questions.sql with {len(records)} questions!")
