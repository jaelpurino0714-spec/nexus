...
new_section_6 = """# --- 6. Parse MCQ Term 2 ---
...
for fname in ['build_master_seed_fast.py', 'generate_master_sql.py', 'build_clean_seed_sql.py']:
    ...
    # Find where section 6 starts and replace to end of section 6 (before section 7)
    pos_sec6 = code.find("# --- 6. Parse MCQ Term 2 ---")
    pos_sec7 = code.find("# --- 7. Parse MCQ Term 3 ---")
    
    if pos_sec6 != -1 and pos_sec7 != -1:
        code = code[:pos_sec6] + new_section_6 + "\n\n" + code[pos_sec7:]
        with open(fname, 'w', encoding='utf-8') as f:
            f.write(code)
        print(f"Successfully updated Section 6 in {fname}")
    else:
        print(f"Could not find section markers in {fname}")
