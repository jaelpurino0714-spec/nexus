...
const TeacherPortal = {
  ...
  // 2. MY CLASSES WORKFLOW
  openMyClasses() {
    const modal = this.ensureModalContainer('myClassesModal');
    const classes = this.getClassesData();
    ...
    if (classes.length === 0) {
      listHtml = `
        <div style="text-align: center; color: #64748B; padding: 24px 0;">
          <div style="font-size: 2.5rem; margin-bottom: 8px;">🏫</div>
          <p>No active classes created yet! Click <b>+ Create New Class</b> to generate a Class Code.</p>
        </div>
      `;
    } else {
      listHtml = classes.map(c => `
        <div style="background: #F8FAFC; border: 1px solid #E2E8F0; padding: 14px; border-radius: 12px; margin-bottom: 10px;">
          <div style="display: flex; justify-content: space-between; align-items: center;">
            <div>
              <h4 style="margin: 0 0 2px 0; color: #1E293B;">${c.className}</h4>
              <span style="font-size: 0.8rem; color: #6D28D9; font-weight: bold;">CLASS CODE: ${c.code}</span>
            </div>
            <span style="font-size: 0.8rem; background: #E2E8F0; padding: 4px 10px; border-radius: 12px;">${c.studentsCount || 0} Students</span>
          </div>
        </div>
      `).join('');
    }
    ...
    modal.innerHTML = `
      <div class="modal-card" style="max-width: 580px; width: 90%;">
        <div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 16px;">
          <h3 style="margin: 0; color: #4C1D95;">🏫 My Classes Workspace</h3>
          <button class="close-modal-btn" style="background: none; border: none; font-size: 1.3rem; cursor: pointer; color: #666;" onclick="TeacherPortal.closeModal('myClassesModal')">✕</button>
        </div>

        <button class="primary-btn" style="width: 100%; margin-bottom: 16px; background: linear-gradient(135deg, #4F46E5, #3730A3);" onclick="TeacherPortal.promptCreateClass()">
          + Create New Class 🏫
        </button>

        <div class="classes-list-section">
          <h4 style="margin: 0 0 10px 0; color: #334155;">Active Classes (${classes.length})</h4>
          <div style="max-height: 320px; overflow-y: auto;">
            ${listHtml}
          </div>
        </div>
  ...
  getClassesData() {
    const raw = localStorage.getItem('nexus_teacher_classes');
    return raw ? JSON.parse(raw) : [];
  },

  promptCreateClass() {
    const name = prompt('Enter Class Name (e.g. Grade 10 - Einstein):');
    ...
    localStorage.setItem('nexus_teacher_classes', JSON.stringify(classes));
    alert(`Class "${name}" created! Class Code: ${code}`);
    this.openMyClasses();
  },
