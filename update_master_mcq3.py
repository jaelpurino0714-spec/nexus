...
for fname in ['build_master_seed_fast.py', 'generate_master_sql.py', 'build_clean_seed_sql.py']:
    ...
    # 1. Allow all q_type_id for t3_ in add_q
    old_check = "if top_key.startswith('t3_') and q_type_id not in [2, 3]: return"
    new_check = "# Allow all question types for t3_"
    if old_check in code:
        code = code.replace(old_check, new_check)

    # 2. Add or replace Section 7
    ...
    with open(fname, 'w', encoding='utf-8') as f:
        f.write(code)
    print(f"Updated {fname} with Section 7 Term 3 MCQ parser.")
